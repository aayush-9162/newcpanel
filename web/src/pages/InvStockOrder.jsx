// Faithful clone of /auth/inv/stockorder — items with sales but no stock.
...
export default function InvStockOrder() {
  ...
  return (
    <>
      <Topbar title="Stock Order" subtitle="Sold but out of stock and not on order" />
      <div className="flex flex-1 flex-col gap-5 p-5 animate-fade-in">
        <Card>
          <CardContent className="flex flex-wrap items-center gap-3 p-3">
            ...
            <Button variant="outline" size="sm" onClick={() => rowsToCSV(rows, `stockorder-${vendor}.csv`)} disabled={!rows.length}>Export CSV</Button>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <DataTable data={rows} columns={cols} loading={isLoading} pageSize={50} emptyText={vendor ? 'No stock-order candidates.' : 'Pick a vendor.'} />
          </CardContent>
        </Card>
      </div>
    </>
