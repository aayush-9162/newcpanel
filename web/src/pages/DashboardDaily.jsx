  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard
          ...
          onClick={openDetail({
            ...
            detailsEmpty: 'No sales yesterday',
            // Click a sale → its item lines.
            onRowClick: (row) => ({
              title: `Sale ${row.SaleNo}${row.CustomerName ? ' · ' + row.CustomerName : ''}`,
              icon: Boxes,
              accent: 'sky',
              subtitle: `Items on this sale${row.City ? ' · ' + row.City : ''}`,
              detailsDb: 'sql',
              detailsSql: `
                ...
                FROM (
                  ...
                  WHERE CAST(SaleNo AS VARCHAR(20)) = '${String(row.SaleNo).replace(/'/g, "''")}'
                ) t
                ORDER BY ItemID`,
              detailsColumns: [
      ...
      <TopSalespersons
        ...
        hint="UPS · Tickets · Closing · written sales — from the UPS system"
